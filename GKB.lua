...
A Stupid Convert By: Kanujinho

NOTE:
This Script is not mine, i just convert scripts require to localscript
Please Give the Credits of the Convert if you publish the script, or showcase

And... Yes. i Fixed the Eye on the model, DARKECCENTRIC PLS FIX THE EYE OF GENTLEMAN!
...
Idle Music: M3chanicalC0nD4mnation From the Game "Epic Battle Fantasy V"

Original Meltdown Music: Max Steel El Dominio de los Elementos Soundtrack

Alt Meltdown Music (Infinite Script Battlegrounds version): Helix - Archetype

]]

local GENTLEMANTHINGS = game:GetObjects("rbxassetid://79997923306343")[1] -- dont report this model, or i will call my gang (sorry)
GENTLEMANTHINGS.Parent = workspace

local TweenService = game:GetService("TweenService")
FELOADLIBRARY = {}
loadstring(game:GetObjects("rbxassetid://5209815302")[1].Source)() -- IDK i just pick this on the gentleman killbot convert by pendulum hub
local RbxUtility = FELOADLIBRARY
local Create = RbxUtility.Create

local Player = game:GetService("Players").LocalPlayer

EffectPack = GENTLEMANTHINGS.Extras:Clone()
GENTLEMANTHINGS.Extras:Destroy()
...
local lastid= "http://www.roblox.com/asset/?id=2984992418"
...
SpinnyThingyWeld=weld(GENTLEMANTHINGS.Armor.MiniGun.SpinPos,GENTLEMANTHINGS.Armor.MiniGun.SpinPos,SpinnyThingy,cf(0,0,0))
--------------------------------------------Hand
...
for _,v in pairs(GENTLEMANTHINGS.Armor:children()) do
	if v:IsA("Model") then
...
SetTween(LH,{C0=CFrame.new(-.5, -1, 0)*angles(math.rad(0),math.rad(20),math.rad(0))},"Back","Out",animtime) 
		...
		Swait(animtime*30)
		Swait()
		attack = false -- Fix the loop taunt
    end
end
...
sine = sine + 1 -- Fix The Idle animation.
...
local Xgui = playerGui:FindFirstChild("Xgui")
local FAT = Xgui:FindFirstChild("FAT")
local viewport = FAT:FindFirstChild("ViewportFrame")

pcall(function()
    FAT.Active = false
    FAT.Selectable = false
    viewport.Active = false
    viewport.Selectable = false
end)

local function updateMouseVars()
    local okHit, hit = pcall(function() return mouse.Hit end)
    local okTarget, target = pcall(function() return mouse.Target end)
    if okHit and hit then MHIT = hit end
    if okTarget and target then Target = target end
    if okHit and hit and hit.p then _G.XguiMousePos = hit.p end
    _G.XguiMouseHit = (okHit and hit) and hit or _G.XguiMouseHit
    _G.XguiMouseTarget = (okTarget and target) and target or _G.XguiMouseTarget
end
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        updateMouseVars()
        Hold = true
        if type(ClickCombo) == "function" then
            pcall(ClickCombo)
        elseif type(Button1DownF) == "function" then
            pcall(Button1DownF)
        elseif type(KeyDownF) == "function" then
            pcall(function() KeyDownF("mouse1") end)
        end
    end
end)
...
UserInputService.InputEnded:Connect(function(input, gameProcessed)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        updateMouseVars()
        Hold = false
        if type(Button1UpF) == "function" then
            pcall(Button1UpF)
        elseif type(KeyUpF) == "function" then
            pcall(function() KeyUpF("mouse1") end)
        end
    end
end)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if type(KeyDownF) == "function" then
        local ok, err = pcall(function() KeyDownF(k) 
		end)
    else
        if type(KeyDown) == "function" then
            local ok, err = pcall(function() KeyDown(k) 
			end)
        end
    end
end)
...
UserInputService.InputEnded:Connect(function(input, gameProcessed)
    ...
    local k = keyToString(input)
    if type(KeyUpF) == "function" then
        local ok, err = pcall(function() KeyUpF(k) end)
    else
        if type(KeyUp) == "function" then
            local ok, err = pcall(function() KeyUp(k) end)
        end
    end
end)

if not isfile("M3chanicalC0nD4mnation.mp3") then
writefile("M3chanicalC0nD4mnation.mp3", game:HttpGet("https://github.com/Kanujinho/Soundtrack/raw/refs/heads/main/M3chanicalC0nD4mnation.mp3"))
end

if not isfile("ReactorCore.mp3") then
writefile("ReactorCore.mp3", game:HttpGet("https://github.com/paravid/Music-Scripts/raw/refs/heads/main/ReactorCore.mp3"))
end

Music = Instance.new("Sound")
Music.SoundId = getcustomasset("M3chanicalC0nD4mnation.mp3") -- MASTERPIECE
Music.Volume = 2
Music.Looped = true
Music.Parent = game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart")
Music:Play()

Meltdown = Instance.new("Sound")
Meltdown.SoundId = getcustomasset("ReactorCore.mp3") -- MASTERPIECE V2
Meltdown.Volume = 5
Meltdown.Looped = true
Meltdown.Parent = game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart")