-- //The Boys (Edited) By: kanujinho\\ --

script = game:GetObjects("rbxassetid://7473297462")[1].BoysAreBack
SCRIPTUTIL = game:GetObjects("rbxassetid://5276526474")[1]
GlobalFunctions = {}
...
local ScreamAction = false;
local TauntAction = false;
local TeleportAction = false;
local Taunt2Action = false;
local targetnnn = nil;
...
for _, v in ipairs(Axe:GetDescendants()) do
    if v:IsA("BasePart") then
        v.Massless = true
        v.CanCollide = false
		v.Transparency = 1
...
function Taunt2()
	...
	NeutralAnims = false
	WalkSpeed = 0
	LETHMIN = Sound(Head,198165368,1,0,false,false,false)
	LETHMIN:Play()
	while LETHMIN.Playing do
	local Alpha = .2
	    Animate('RS',CF.N(1.43,0.46,0.11)*CF.A(M.R(-60.2),M.R(2.2),M.R(-45))*CF.A(M.RRNG(-5,5),M.RRNG(-5,5),M.RRNG(-5,5)),.33, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
	    Animate("LS",CF.N(-1.5,0.23,-0.04)*CF.A(M.R(73.3-20*M.S(Sine/24)+12*M.C(Sine/24)),M.R(-20.1),M.R(0)),.33, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
	   fwait(27)
    end
        WalkSpeed = 16
	NeutralAnims = true
...
function Taunt()
	...
	NeutralAnims = false
	WalkSpeed = 0
	Crack = Sound(Head,198165368,2,0,false,false,false)
	Crack:Play()
	while Crack.Playing do
	local Alpha = .2
		Animate("RS",CF.N(1.15, 0.55 - 0.075 * M.S(Sine/14), -0.1)* CF.A(M.R(78.3 - 5 * M.S(Sine/12)), M.R(0 - 15 * M.S(Sine/12)), M.R(-50)),.33, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
		Animate("LS",CF.N(-1.15, 0.55 - 0.075 * M.S(Sine/14), -0.1)* CF.A(M.R(78.3 - 5 * M.S(Sine/12)), M.R(0 + 15 * M.S(Sine/12)), M.R(40)),.33, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
	   fwait(27)
    end
        WalkSpeed = 16
	NeutralAnims = true
		...
		while Attack do
			AoE(RLeg.CFrame.p,3,function(char,hum)
				if(hum.Health>0)then
					...
					if(tor)then
						Sound(tor,1,1,2,false,true,true)
					end
				end
			end)
...
function Scream()
	...
	NeutralAnims = false
	WalkSpeed = 0
        Snd = Sound(Head,198165368,0.65,0,false,false,false)
	Snd:Play()
...
function Behead()
	...
	NeutralAnims = false
	WalkSpeed = 4
	local swoosh = Sound(Root,1,1.2,2,false,true,false)
...
function Cleave(key)
	...
	while game:service'UserInputService':IsKeyDown(key)do
		...
		swingNum=currSwing
		local swoosh = Sound(Root,1,1.5,2,false,true,false)
		--swing start--
