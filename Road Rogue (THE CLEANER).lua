...
Button.Size = UDim2.new(0, 190, 0, 60)
Button.Position = UDim2.new(0.75, -120, 0.91, 0)
...
Button.MouseButton1Click:Connect(function()
	...
	local backwards = -root.CFrame.LookVector

	local velocity = Instance.new("BodyVelocity")
	velocity.MaxForce = Vector3.new(1e6, 1e6, 1e6)
	velocity.Velocity = backwards * 150 + Vector3.new(0, 40, 0)
...
for _,v in ipairs(BACKWHEEL:GetDescendants()) do
	if v:IsA("BasePart") then
		v.Transparency = 1
	end
end
for _,v in ipairs(FRONTWHEEL:GetDescendants()) do
	if v:IsA("BasePart") then
		v.Transparency = 1
	end
end
...
local MOTORHUM = NewSound({ID = 1,PARENT = ROOT,VOLUME = 0,PITCH = 1,LOOP = true,MAXDISTANCE = 125,EMITTERSIZE = 15,PLAYING = true,PLAYONREMOVE = false,DOESDEBRIS = true})
...
local MOTORHIT = MOTORHITBOX.Touched:Connect(function(HIT)
	if HUM.WalkSpeed > 80 then
		if HIT.Parent:FindFirstChildOfClass("Humanoid") and not HIT:FindFirstAncestor('non') and not HIT:FindFirstAncestor(game.Players.LocalPlayer.Name) then
			if HIT.Parent:FindFirstChildOfClass("Humanoid").Health > 100 and HIT.Parent:FindFirstChild("Roadrogue") == nil then
...
local M = NewSound({ID = 2,PARENT = ROOT,VOLUME = 2.75,PITCH = 1,LOOP = true,MAXDISTANCE = 300,EMITTERSIZE = 300,PLAYING = true,PLAYONREMOVE = false,DOESDEBRIS = true})
M.Name = "BGM_MUSIC"
...
local f = game:GetService("RunService").RenderStepped:Connect(function()
	...
	if ROOT:FindFirstChild("MOTORHUM") == nil then
		MOTORHUM = NewSound({ID = 1,PARENT = ROOT,VOLUME = 0,PITCH = 1,LOOP = true,MAXDISTANCE = 125,EMITTERSIZE = 15,PLAYING = true,PLAYONREMOVE = false,DOESDEBRIS = true})
		MOTORHUM.Name = "MOTORHUM"
	else
